...
import { magicSchema, passwordSchema } from "../../schemas/auth.schema";
...
export default function Login() {
  ...
  const {
    ...
    resolver: zodResolver(
      mode === "magic" ? magicSchema : passwordSchema(isRegister)
    ) as unknown as Resolver<PasswordFormData>,
  });

  //FORMAS DE LOGIN
  const handleEmailAuth = async ({ email, password }: EmailLogin) => {
  ...
  const handleMagicLink = async ({ email }: MagicLogin) => {
    setLoading(true);
      ...
      <div className="relative w-full max-w-sm bg-[#0f1611]/85 border border-indigo-500/20 rounded-2xl px-9 py-10 backdrop-blur-xl shadow-[0_32px_64px_rgba(0,0,0,0.5),0_0_80px_rgba(16,185,129,0.05)] animate-fadeUp">
        ...
        <div className="space-y-3.5">
          ...
          {mode === "password" && (
            <div>
              ...
              {/*CONFIRMAR PASSWOD SOLO EN REGISTRO CREA UNA CUENTA*/}
              {isRegister && (
                <>
        ...
        <button
          onClick={handleSubmit(
            mode === "magic" ? handleMagicLink : handleEmailAuth
          )}
          disabled={loading}
